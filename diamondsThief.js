...
function diamondBurglar(locker) {
  ...
  for (let row of locker) {
    if (row.includes("*") && !row.includes(null)) {
      let singleDiamondsPerRow = row.match(/\*/g).length;
      result += singleDiamondsPerRow;
    }
    if (row.match(/\*\*/g) && !row.includes(null)) {
      let pairedDiamondsPerRow = row.match(/\*\*/g).length;
      result -= pairedDiamondsPerRow;
    }
  }
  return result;
}
